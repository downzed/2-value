...
const FloatingControls: React.FC = () => {
	const { hasImage, blur, threshold, invert, setBlur, setThreshold, setInvert, resetControls } = useImageContext();
		...
		>
			...
			>
				<div className='flex items-center gap-2'>
					...
					<span className='text-xs font-semibold text-slate-700'>Adjustments</span>
				</div>
			...
			<div className='p-3 space-y-3'>
				...
				<div className='flex items-center gap-2'>
					...
					<input
						...
						value={threshold}
						onChange={(e) => debouncedSetThreshold(parseInt(e.target.value, 10))}
						disabled={!hasImage}
						className='flex-1 h-1.5 bg-slate-200 rounded-lg appearance-none cursor-pointer disabled:opacity-50'
					/>
