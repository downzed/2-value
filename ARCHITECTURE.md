...
Pinterest integration enables users to authenticate via OAuth 2.0, browse their boards, and save edited images directly as Pinterest pins — all without leaving the app.

### Security Model

```
┌─ Main Process (Full Node.js Access)
│  ├─ pinterest-auth.ts  – OAuth flow, token exchange, safeStorage encryption
│  ├─ pinterest-api.ts   – Pinterest API v5 HTTP wrapper
│  └─ index.ts           – IPC handlers (pinterest:auth, pinterest:get-boards, etc.)
│
├─ Preload Bridge
│  └─ Exposes: pinterestAuth, pinterestAuthStatus, pinterestLogout,
│              pinterestGetBoards, pinterestSavePin
│
└─ Renderer Process (No direct Pinterest access)
   ├─ context/PinterestContext.tsx  – boards, auth status, save action
   ├─ hooks/usePinterestAuth.ts     – auth state management
   └─ components/PinterestPanel.tsx – floating UI panel
```

**Key principles:**
- `CLIENT_ID` and `CLIENT_SECRET` never leave the main process — they are read from environment variables.
- Access tokens are encrypted at rest using `safeStorage.encryptString()` and stored in `userData/pinterest-token.enc`.
- The renderer only sends `imageDataUrl` + `boardId` over IPC; the main process performs all API calls.
- A random `state` parameter (with 10-minute TTL) protects against CSRF during the OAuth flow.
...
User clicks "Connect Pinterest"
    ↓
PinterestPanel → pinterestAuth() IPC
    ↓
[IPC Boundary]
    ↓
Main: initiateOAuthFlow()
    ├─ Generates random state (CSRF protection)
    ├─ Builds authorization URL with scopes: boards:read, pins:read, pins:write
    └─ shell.openExternal(authUrl) → system browser
    ↓
User authorizes in browser → Pinterest redirects to imageapp://pinterest/callback
    ↓
Main: handleOAuthCallback(code, state)
    ├─ Validates state parameter
    ├─ POST /oauth/token (code exchange)
    └─ safeStorage.encryptString(token) → userData/pinterest-token.enc
    ↓
pinterestAuthStatus() IPC → { authenticated: true }
    ↓
PinterestContext: isAuthenticated = true → fetchBoards()
    ↓
PinterestPanel: shows board list + "Save as Pin" button
```

### Save to Pinterest Flow

```
User selects a board + clicks "Save as Pin"
    ↓
PinterestPanel.handleSaveToBoard()
    ├─ canvas.toDataURL('image/png') → base64 data URL
    └─ pinterestSavePin({ boardId, imageDataUrl }) IPC
    ↓
[IPC Boundary]
    ↓
Main: savePin()
    ...
    └─ POST /v5/pins with media_source: { source_type: "image_base64", data }
    ↓
{ ok: true, pin: PinterestPin } → success feedback in UI
```
...
src/
├── shared/
│   └── pinterest-types.ts         TypeScript interfaces (PinterestBoard, PinterestPin, …)
├── main/
│   ├── pinterest-auth.ts          OAuth 2.0 flow + safeStorage token management
│   ├── pinterest-api.ts           Pinterest API v5 wrapper (boards, pins)
│   └── index.ts                   IPC handlers: pinterest:auth, :get-boards, :save-pin, …
├── preload/
│   └── index.ts                   Exposes pinterestAuth*, pinterestGetBoards, pinterestSavePin
└── renderer/
    ├── constants/pinterest.ts     Scopes, default panel position, storage key
    ├── context/PinterestContext.tsx  React context: boards, auth, savePin action
    ├── hooks/usePinterestAuth.ts  Auth state hook
    └── components/
        ├── PinterestPanel.tsx     Floating UI panel (FloatingWidget wrapper)
        └── shell/
            ├── App.tsx            Wraps AppContent in PinterestProvider
            └── BottomPanel.tsx    Pinterest icon in minimized-panel strip
```
...
window.electronAPI = {
  ...
  pinterestAuthCallback: (code, state) → Promise<PinterestAuthResult>,
...
Register your app at <https://developers.pinterest.com/apps/> and set the redirect URI to `imageapp://pinterest/callback`.

### Future Phases

| Phase | Description |
|-------|-------------|
| Phase 2 | Register `imageapp://` custom protocol handler; wire full OAuth callback |
| Phase 3 | Board management UI (create board, search boards) |
| Phase 4 | Pin upload with title/description/link editing |
| Phase 5 | Load inspiration images from boards into the editor |
| Phase 6 | E2E tests with Pinterest API mock |
